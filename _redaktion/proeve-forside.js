...
const complete=JSON.parse(fs.readFileSync(path.join(repo,'_redaktion/fixtures/faerdig-artikel.json'),'utf8'));
const completed = a => ({...a,sektioner:complete.sektioner,publicering:{status:'godkendt'}});
...
(async()=>{
  ...
  const sunoFixture=JSON.parse(fs.readFileSync(path.join(repo,'_redaktion/fixtures/suno-v6.json'),'utf8')).map(completed);
  ...
  const input=d.getElementById('soeg');const searchTerm=real.artikler.find(a=>api.publishable(a)).rubrik.split(' ').slice(0,2).join(' ').toLowerCase();input.value=searchTerm;input.dispatchEvent(new w.Event('input'));await pause(150);
  ...
  searchButton.click();
  ok(input.value===searchTerm,'Søgeord bevares efter genåbning');
  ...
  const hostileArticle={...real.artikler[0],rubrik:'<img src=x onerror=alert(1)>',dato:new Date(testNow).toISOString(),billede:'javascript:alert(1)',sektioner:complete.sektioner.map(s=>({...s,overskrift:'<script>x</script>',tekst:s.tekst+' <img onerror=x>'})),andre:[null],figurer:[null]};
  const unfinished=await mount({artikler:[{...real.artikler[0],sektioner:[]}],opdateret:new Date().toISOString()});
  ok(!unfinished.w.document.querySelector('a[data-article]'),'Et kort udkast må ikke være synligt på forsiden');ok(!unfinished.errors.length,'Ufærdig artikel filtreres uden scriptfejl');unfinished.close();
