...
(function () {
  ...
  function images() {
    ...
    <div><section class="panel image-status">${panelHead('Den visuelle dækning','Eksisterende illustrationer i den seneste udgave')}<div class="panel-body"><div class="split-summary"><div><strong>${fmt(a.med_billede)}</strong><span>med billede</span></div><div><strong>${fmt(a.antal)}</strong><span>historier i alt</span></div></div><div class="meter-track" role="meter" aria-label="Andel historier med billede" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${count}"><span class="meter-fill" style="width:${count}%"></span></div><div class="status-row"><span class="stat-icon">${icon('image')}</span><div><strong>Generator ved seneste kørsel</strong><small>${esc(h.billedmodel||'Ikke målt')}</small></div></div><p class="help">En ny billedinstruktion bruges til nye billeder. Allerede gemte illustrationer bliver bevaret.</p></div></section><section class="panel">${panelHead('FLUX.2 Klein 4B','Cloudflare Workers AI')}<div class="panel-body"><p>FLUX.2 Klein 4B er tilsluttet i crawleren. Kræver Cloudflare-token med Workers AI-adgang. Eksisterende billeder bevares.</p><p class="help">Denne central viser kun indstillinger, som crawleren faktisk bruger.</p></div></section></div></div>`;
  }
  function readers() {
