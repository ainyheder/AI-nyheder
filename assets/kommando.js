...
(function () {
  ...
  const STEP_NAMES = {forside_agent:'Vælg forsiden · redaktøragent',billedgenerator:'Generér illustrationer',omskriv:'Rubrik og resumé',kategori:'Vurder nyhedsværdi',dublet:'Saml dubletter',brief:'Skriv hele artiklen',redaktoer:'Kontrollér artiklen',stram:'Stram teksten op',navngiv:'Forbedr gamle rubrikker',motiv:'Beskriv billedmotivet',dagens_overblik:'Dagens overblik',ugens_overblik:'Ugens overblik',nyhedsbrev:'Skriv nyhedsbrevet',nyhedsbrev_kontrol:'Kontrollér nyhedsbrevet',opslag:'Skriv sociale opslag'};
  const VIEWS = {
  ...
  function modelDefault(name) {
    const h=brainStatus();
    return name.startsWith('nyhedsbrev') ? (state.snapshot.nyhedsbrev?.model||'deepseek-flash') : name==='forside_agent' ? (h.forside_standard||'deepseek-flash') : name==='billedgenerator' ? (h.billed_standard||'gemini-3.1-flash-lite-image') : h.daglig_model||'';
  }
